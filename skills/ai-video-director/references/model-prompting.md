...
- 先确定目标模型，再用该模型的写法要求填充三段式提示词。
- 不要先输出一份“通用三段式”，再额外输出一份“模型改写版”；这会造成重复和割裂。
- 指定单个模型时，每个镜头只输出该模型可直接使用的一套三段式提示词。
- 指定多个模型时，同一镜头分别输出各模型的三段式版本。
...
适合“分层导演式提示词”。重点写清主体、动作、场景、镜头、风格、声音和格式。
...
- 复杂多镜头项目建议拆成单镜头生成，不要一次让模型完成完整短片。
...
```text
Create a short {duration} video.
Subject: {clear subject}.
Action: {one main action}.
Scene: {simple environment}.
Camera: {one camera style or movement}.
...
```text
Generate a {duration} {aspect ratio} cinematic video.
Subject: {who/what, appearance, stable details}.
Action: {what happens, sequence of movement}.
Scene: {location, time, weather, props, background}.
Camera: {shot size, lens, composition, movement}.
...
```text
A {duration} video of {subject} {action} in {scene}.
...
```text
{Subject} {action} in {scene}. {Camera movement}. {Lighting/style}. {Mood}.
```
...
The subject {motion}. The camera {movement}. The background {subtle motion}.
...
- 改成 Veo：增加声音、时间点、镜头和美学字段。
- 改成 Seedance：改为中文分层提示词，强化主体、动作、镜头、声音和格式。
- 改成 Runway 图生视频：删掉多余静态描述，强调参考图保持项和运动项。
- 改成 Sora：改成自然语言场景描述，复杂内容拆 Storyboard。
- 改成 Kling：压缩成主体、运动、场景、镜头、光线、氛围。
- 改成 Grok：缩短，突出一个主体、一个动作、一个镜头目标。
